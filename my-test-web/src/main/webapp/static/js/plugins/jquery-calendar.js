/**
 * @fileoverview dynDateTime 0.2基于jQuery的时间插件，提供各种时间的选择
 * @author Toolman {@link http://code.google.com/p/dyndatetime/}
 * @version 0.2
 * @example
...
(function ($) {
    ...
    $.fn.dynDateTime = function (options) {
        ...
        var opts = $.extend({},
        $.fn.dynDateTime.defaults, options);
        return this.each(function () {
            ...
            function onSelect(cal) {
                ...
                if (update && this_inputField) {
                    ...
                    /*检查清空标志inputClear，清空输入框*/
                    if (typeof this_inputField.onchange == "function") this_inputField.onchange();
                }
                if (update && this_displayArea) this_displayArea.innerHTML = cal.date.print(p.daFormat);
            ...
            triggerEl["on" + opts.eventName] = function () {
                ...
                if (!(cal && opts.cache)) {
                    window.calendar = cal = new Calendar(opts.firstDay, opts.date, opts.onSelect || onSelect, opts.onClose ||
                    function (cal) {
                        cal.hide();
                    });
                    cal.showsTime = opts.showsTime;
                    ...
                    if (opts.date) cal.setDate(opts.date);
                    cal.hide();
                }
                if (opts.multiple) {
                    cal.multiple = {};
                ...
                cal.refresh();
                if (!opts.position) cal.showAtElement(this_button || this_displayArea || this_inputField, opts.align);
                else cal.showAt(opts.position[0], opts.position[1]);
                return false;
            };
        });
...
Calendar.timeChange = function (ev) {
    var el = Calendar.getElement(ev);
    if (el.disabled) {
        return false;
    }
    ...
    if (el.navtype != 300) with (Calendar) {
        if (el.navtype == 150) {
            el._current = el.value;
            //addEvent(document, "mousemove", tableMouseOver);
        } else
        //addEvent(document, Calendar.is_ie5 ? "mousemove" : "mouseover", tableMouseOver);
            addClass(el, "hilite active"); //加active类，知道激活的元素，以便获取它的值
        cal.onUpdateTime(); //设置时间
        //addEvent(document, "mouseup", tableMouseUp);
    } else if (cal.isPopup) {
        cal._dragStart(ev);
    }
...
Calendar.prototype.create = function (_par) {
    ...
    var title_length = 6; (this.isPopup) && --title_length; (this.weekNumbers) && ++title_length;

    hh("清空", 1, 400).ttip = Calendar._TT["INFO"];
    /*原为hh("?", 1, 400).ttip = Calendar._TT["INFO"];*/
    ...
    if (this.showsTime) {
        ...
        (function () {
            ...
            function makeTimePart(className, init, range_start, range_end) {
                ...
                part.value = init;
                part.calendar = cal;
                part.setAttribute("navtype",150);
                part.setAttribute("calendar",cal);
                part.setAttribute("ttip",Calendar._TT["TIME_PART"]);
                part._range = [];

                for (var i = 0; i <= range_end; i++) { //添加小时, 分
                    if (i < 10) {
                ...
                //Calendar._add_evs(part);
                with (Calendar) {
                    Calendar.addEvent(part, "change", timeChange);
                }
                return part;
            };
...
Calendar._keyEvent = function (ev) {
    ...
    if (!cal || cal.multiple) return false; (Calendar.is_ie) && (ev = window.event);
...
Calendar.prototype.hide = function () {
    ...
    this.hidden = true;
    this.hideShowCovered();
    this.element.parentNode.removeChild(this.element);//2011/11/25关闭时删除时间控件层
};
...
Calendar.prototype.showAtElement = function (el, opts) {
    ...
    function fixPosition(box) {
        ...
        if (Calendar.is_ie) {//修复ie有滚动条时位置错误。by fengweiping
    ...
    Calendar.continuation_for_the_fucking_khtml_browser = function () {
        ...
        //var halign = "l";
        var halign =opts.substr(1, 1);
        //console.log(opts.substr(1, 1));
        if (opts.length > 1) {
        ...
        switch (valign) {
            case "T":
                ...
                break;
            case "B":
                p.y += el.offsetHeight + 3;
                break; //调低3像素，防止ie div覆盖不了select的bug.
            case "C":
    ...
    for (var k = tags.length; k > 0; ) {
        ...
        for (var i = ar.length; i > 0; ) {
            ...
            if (this.hidden || (CX1 > EX2) || (CX2 < EX1) || (CY1 > EY2) || (CY2 < EY1)) {
                ...
                //cc.style.visibility = "hidden";//为了避免select穿透，控件下的select设置隐藏属性
            }
        }
    }
};
...
Date.prototype.print = function (str) {
    ...
    s["%F"] = (wn-1); // getWeekNumber
    var re = /%./g;
    if (!Calendar.is_ie5 && !Calendar.is_khtml) return str.replace(re,
    function (par) {
        return s[par] || par;
    });
...
// full day names
Calendar._DN = new Array("周日", "周一", "周二", "周三", "周四", "周五", "周六", "周日");
...
"Sat",
"Sun");*/
Calendar._SDN = new Array("周日", "周一", "周二", "周三", "周四", "周五", "周六", "周日");
